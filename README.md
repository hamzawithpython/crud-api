...
2. Copy `.env.example` to `.env` and fill in real values:
   ...
   cp .env.example .env
   ```
   - `DATABASE_URL` â€” matches the Postgres container defined in `compose.yaml` (see [Database](#database) below)
   - `SUPABASE_URL` / `SUPABASE_KEY` â€” from your Supabase project's **Settings â†’ API Keys** page. Use the **anon** (legacy) or **publishable** key only â€” never the `service_role`/secret key.
...
Auth is handled by Supabase. Sign up, log in, and receive a JWT access token â€” send it as a `Bearer` token on protected routes. Verification happens via a live call to Supabase (`supabase.auth.get_user(token)`), not local JWT decoding, so revoked/expired tokens are always caught server-side rather than trusted blindly.

The auth check is implemented once as a reusable FastAPI dependency (`get_current_user`) and applied to every protected route â€” no copy-pasted verification logic.
...
**Example â€” calling a protected route:**
```powershell
curl.exe http://127.0.0.1:8000/protected/dashboard -H "Authorization: Bearer <your_access_token>"
```

**Swagger UI:** `/docs` shows a padlock icon on protected routes. Click **Authorize**, paste your access token (no `Bearer` prefix needed â€” Swagger adds it automatically), and call any protected route directly from the browser.
...
See [ENRICH-README.md](ENRICH-README.md) for the POST /enrich endpoint, design decisions, and eval results.
